// ---------------------------------------------------------------------------
// DESIGN B: histogram / radix THRESHOLD selection, optionally fused with the scorer.
//
...
//   P1 digit1 histogram (fusable into the scorer) | S1 scan -> b1, c1, need1
...
//
// Every launch uses at::cuda::getCurrentCUDAStream() and is error-checked. (The pre-existing
// soft_hash_collision.cu launches with no stream argument, i.e. on the LEGACY DEFAULT
// STREAM, so it is never captured as a CUDA-graph node and never runs at replay.)
// ---------------------------------------------------------------------------
#include <cuda.h>
...
// torch's extension build defines __CUDA_NO_HALF_CONVERSIONS__ /
// __CUDA_NO_BFLOAT16_CONVERSIONS__, so operator float() is unavailable. These intrinsics
// are exact widenings, i.e. identical to Triton's `.to(tl.float32)` and to the fp32 copy
// the .cu scorer is fed.
...
// ---------------------------------------------------------------------------
// P1, FUSED form: soft-hash score + write + digit-1 histogram in one pass.
//   score[b,h,t] = ( sum_{l<L} q_probs[b,h,l, buckets[b,kv,l,t]] ) * v_norm[b,kv,t]
// Bit-identical to soft_hash_collision_kernel_3 and to the Triton scorer: the same fp32
// adds in the same l order, then one multiply.  q_probs is read in its native dtype
// (float(bf16) is exact) and v_norm in native fp16, so the [B,H,1,L,R] fp32 copy, the
// [B,Hkv,1,T] fp32 v_hist copy and the [B,H,T] bool `allowed` tensor that the .cu scorer
// needs are all deleted -- the mask is the scalar compare t < seq_len.
//
// LC>0 makes L a compile-time constant so the L-deep gather loop fully unrolls and issues
// all L INDEPENDENT int16 bucket loads before any of the dependent q_probs lookups. With a
// runtime L the loop is a ~400-cycle-per-l dependent chain.
// STAGE_Q>0 stages q_probs[b,h] (L*R floats) in shared memory: that lookup is a random
// gather into a 20-50 KB table performed L times per token, i.e. the only non-streaming
// access in the kernel.
// ---------------------------------------------------------------------------
template <typename QT, typename VT, int LC, int STAGE_Q, int HMODE>
__global__ __launch_bounds__(1024) void rs_score_hist1(const QT* __restrict__ q_probs,      // [B,H,L,R]
                               const short* __restrict__ buckets,   // [B,Hkv,L,T] int16
                               const VT* __restrict__ v_norm,       // [B,Hkv,T]
                               const int* __restrict__ seq_len_p,
                               float* __restrict__ scores,          // [B,H,T] written
                               int H, int HKV, int Ldyn, int R, int T, int NB,
                               unsigned* __restrict__ h1) {
    // COMPILE-TIME shared offsets. Deriving s_q from a runtime replication factor instead
    // measured 82.6 us against 51.6 us for the identical arithmetic: the q_probs table
    // lookup is the hot inner op and a runtime shared base defeats immediate-offset
    // addressing.
    extern __shared__ unsigned smem[];
    unsigned* s_cnt = smem;
    float* s_q = (float*)(smem + NBINS);
    const int L = LC > 0 ? LC : Ldyn;
    const int bh = blockIdx.x, blk = blockIdx.y, BH = gridDim.x;
    const int b = bh / H, h = bh - b * H;
    const int kv = h / (H / HKV);

    const QT* qp = q_probs + ((long)b * H + h) * L * R;
    for (int i = threadIdx.x; i < NBINS; i += blockDim.x) s_cnt[i] = 0u;
    if (STAGE_Q)
        for (int i = threadIdx.x; i < L * R; i += blockDim.x) s_q[i] = to_f(qp[i]);
    __syncthreads();

    const int seq_len = *seq_len_p;
    const short* kb = buckets + ((long)b * HKV + kv) * L * (long)T;
    const VT* vn = v_norm + ((long)b * HKV + kv) * (long)T;
    float* out = scores + (long)bh * (long)T;

    const int per = (T + NB - 1) / NB;
    const int t0 = blk * per, t1 = min(T, t0 + per);
    const int stride = blockDim.x;
    // OUTER unroll over t. Deep-L configs (L=50) must NOT combine a full inner unroll with
    // UNROLL=4: 200 in-flight int16 bucket loads blows past the 64 registers/thread that
    // 1024 threads allow and the launch fails with cudaErrorLaunchOutOfResources.
    constexpr int UNROLL_S = (LC > 0 && LC <= 16) ? UNROLL : 1;
#define SCORE_ONE(tt, ss)                                                          \
    do {                                                                           \
        float acc_ = 0.0f;                                                         \
        if (LC > 0 && LC <= 16) {                                                  \
            _Pragma("unroll")                                                      \
            for (int l = 0; l < (LC > 0 ? LC : 1); ++l) {                          \
                int r_ = (int)kb[(long)l * T + (tt)];                              \
                acc_ += STAGE_Q ? s_q[l * R + r_] : to_f(qp[(long)l * R + r_]);     \
            }                                                                      \
        } else if (LC > 0) {                                                        \
            _Pragma("unroll 10")                                                   \
            for (int l = 0; l < (LC > 0 ? LC : 1); ++l) {                          \
        ...
        (ss) = ((tt) < seq_len) ? acc_ * to_f(vn[(tt)]) : -INFINITY;                \
    } while (0)
    int t = t0 + threadIdx.x;
    for (; t + (UNROLL_S - 1) * stride < t1; t += UNROLL_S * stride) {
        float s[UNROLL_S];
#pragma unroll
        for (int u = 0; u < UNROLL_S; ++u) SCORE_ONE(t + u * stride, s[u]);
#pragma unroll
        for (int u = 0; u < UNROLL_S; ++u) {
            out[t + u * stride] = s[u];
            hist_add<HMODE>(s_cnt, (int)(f2d(s[u]) >> SH1));
        }
    }
    for (; t < t1; t += stride) {
        float s1;
        SCORE_ONE(t, s1);
        out[t] = s1;
        hist_add<HMODE>(s_cnt, (int)(f2d(s1) >> SH1));
    }
#undef SCORE_ONE
    __syncthreads();
    unsigned* o1 = h1 + ((long)blk * BH + bh) * NBINS;
    for (int i = threadIdx.x; i < NBINS; i += blockDim.x) o1[i] = s_cnt[i];
}
...
// FUSED: soft-hash score (+ write to `scores`) + digit-1 histogram + exact top-M select.
void radix_score_select(torch::Tensor q_probs, torch::Tensor buckets, torch::Tensor v_norm,
                        torch::Tensor seq_len, torch::Tensor scores, torch::Tensor out,
                        torch::Tensor ws, int64_t M, int64_t NB, int64_t THR, int64_t STHR,
                        int64_t stages, int64_t hmode, int64_t stage_q, int64_t det) {
    TORCH_CHECK(q_probs.is_cuda() && buckets.is_cuda() && v_norm.is_cuda(), "cuda");
    TORCH_CHECK(q_probs.dim() == 4 && q_probs.is_contiguous(), "q_probs [B,H,L,R] contiguous");
    TORCH_CHECK(buckets.dim() == 4 && buckets.scalar_type() == torch::kShort &&
                buckets.is_contiguous(), "buckets [B,Hkv,L,T] int16 contiguous");
    TORCH_CHECK(v_norm.dim() == 3 && v_norm.is_contiguous(), "v_norm [B,Hkv,T] contiguous");
    TORCH_CHECK(scores.scalar_type() == torch::kFloat && scores.is_contiguous(), "scores fp32");
    TORCH_CHECK(seq_len.scalar_type() == torch::kInt, "seq_len must be int32");
    int B = (int)q_probs.size(0), H = (int)q_probs.size(1);
    int L = (int)q_probs.size(2), R = (int)q_probs.size(3);
    int HKV = (int)buckets.size(1), T = (int)buckets.size(3);
    int BH = B * H;
    common_checks(out, ws, NB, BH, STHR, THR);
    TORCH_CHECK(H % HKV == 0, "n_head must be divisible by n_kv_heads");
    TORCH_CHECK(buckets.size(0) == B && buckets.size(2) == L, "buckets shape");
    TORCH_CHECK(v_norm.size(0) == B && v_norm.size(1) == HKV && v_norm.size(2) == T,
                "v_norm shape");
    TORCH_CHECK(scores.numel() == (long)BH * T, "scores must be [B,H,T]");
    TORCH_CHECK(M <= out.size(2), "M must be <= out.size(2)");
    WS w = split_ws(ws, NB, BH);
    auto st = at::cuda::getCurrentCUDAStream();
    const short* kb = reinterpret_cast<const short*>(buckets.data_ptr<int16_t>());
    const int* sl = seq_len.data_ptr<int32_t>();
    float* sc = scores.data_ptr<float>();

    if (stages & 1) {
        size_t shb = (size_t)NBINS * 4;
        // stage q_probs in shared memory only if it fits the 99 KB we opt into
        int sq = (stage_q && (shb + (size_t)L * R * 4 <= 99u * 1024u)) ? 1 : 0;
        if (sq) shb += (size_t)L * R * 4;
        dim3 g((unsigned)BH, (unsigned)NB, 1), blk((unsigned)THR, 1, 1);
        bool launched = false;
#define LAUNCH(QTY, VTY, qt, vt, LCV, SQ, HM)                                             \
        if (!launched && q_probs.scalar_type() == (qt) && v_norm.scalar_type() == (vt) &&  \
            ((LCV) == 0 || L == (LCV)) && sq == (SQ) && hmode == (HM)) {                   \
            auto kfn = rs_score_hist1<QTY, VTY, LCV, SQ, HM>;                              \
            if (shb > 48u * 1024u)                                                         \
                cudaFuncSetAttribute(kfn, cudaFuncAttributeMaxDynamicSharedMemorySize,     \
                                     101376);                                              \
            kfn<<<g, blk, (unsigned)shb, st>>>(                                            \
                reinterpret_cast<const QTY*>(q_probs.data_ptr()), kb,                      \
                reinterpret_cast<const VTY*>(v_norm.data_ptr()), sl, sc,                   \
                H, HKV, L, R, T, (int)NB, w.h1);                                           \
            C10_CUDA_KERNEL_LAUNCH_CHECK(); launched = true;                               \
        }
#define LAUNCH_L(QTY, VTY, qt, vt, SQ, HM)  \
        LAUNCH(QTY, VTY, qt, vt, 10, SQ, HM) \
        LAUNCH(QTY, VTY, qt, vt, 50, SQ, HM) \
        LAUNCH(QTY, VTY, qt, vt,  0, SQ, HM)
#define LAUNCH_SQ(QTY, VTY, qt, vt, HM) \
        LAUNCH_L(QTY, VTY, qt, vt, 0, HM) \
        LAUNCH_L(QTY, VTY, qt, vt, 1, HM)
#define LAUNCH_HM(QTY, VTY, qt, vt) \
        LAUNCH_SQ(QTY, VTY, qt, vt, 1) \
        LAUNCH_SQ(QTY, VTY, qt, vt, 3)
        LAUNCH_HM(__nv_bfloat16, __half, torch::kBFloat16, torch::kHalf)
        LAUNCH_HM(float, float, torch::kFloat, torch::kFloat)
#undef LAUNCH_HM
#undef LAUNCH_SQ
#undef LAUNCH_L
#undef LAUNCH
        TORCH_CHECK(launched, "unsupported (q_probs,v_norm) dtype pair or hmode for the "
                              "fused scorer (supported: bf16/fp16 and fp32/fp32; hmode 1 or 3)");
    }
    run_select(sc, T, BH, (int)M, (int)out.size(2), (int)NB, (int)THR, (int)STHR, w,
               out.data_ptr<int32_t>(), st, ((int)stages) & ~1, (int)hmode, (int)det);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("radix_score_select", &radix_score_select, "fused soft-hash score + exact top-M",
          pybind11::arg("q_probs"), pybind11::arg("buckets"), pybind11::arg("v_norm"),
          pybind11::arg("seq_len"), pybind11::arg("scores"), pybind11::arg("out"),
          pybind11::arg("ws"), pybind11::arg("M"), pybind11::arg("NB"), pybind11::arg("THR"),
          pybind11::arg("STHR"), pybind11::arg("stages") = 127,
          pybind11::arg("hmode") = 1, pybind11::arg("stage_q") = 1,
          pybind11::arg("det") = 0);
