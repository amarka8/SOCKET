...
def _fwd_kernel_sparse_decode_stage1(
    ...
    for start_n in range(0, block_n_size, 1):
        ...
        # seen yet) new_max_logic is -inf and BOTH exp() calls below evaluate exp(-inf - -inf)
        # = exp(nan) = nan, which then poisons acc/sum_exp for the whole block and propagates
        # through stage2 to the logits.
        # This is a LATENT PRE-EXISTING BUG, not one introduced by the reordering: it simply
        # never fired while topk returned the list in descending-score order, because the -1
        # dedup holes then landed in positions that never filled a whole chunk. Selecting the
        # SAME SET in a different order (topk sorted=False) makes a fully-padded chunk likely
        # and the model emits all-NaN logits (greedy output collapses to token 0).
        # Substituting 0.0 for the max when the running max is still -inf is exact: every
