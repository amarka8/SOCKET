...
CUDA_VISIBLE_DEVICES=0,1,2 deepspeed  --master_port 29666 pipeline/train_quest/main.py \
    --exp_desc ${task}_${train_dataset}_${model}_${method} \
    --pipeline_config_dir config/pipeline_config/${method}/${model}/${model}-traink.json \
    ...
    --deepspeed_config config/deepspeed_config/zero3.json
